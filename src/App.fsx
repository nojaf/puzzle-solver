...
let useReducer<'state, 'msg> (reducer: ReduceFn<'state, 'msg>) (initialState: 'state): ('state * Dispatch<'msg>) =
    import "useReducer" "react"
...
let mimimumSwap (input: int array) =
    let annotated =
        input |> Array.indexed |> Array.sortBy snd
    ...
    seq {
        while (i < length) do
            if (fst annotated.[i]) = i then
                ...
                Array.swap annotated i (fst annotated.[i])
                |> ignore
    }
    |> Seq.toList
    |> List.rev
...
let update model msg =
    printfn "%A" msg
    match msg with
    | FirstTileClicked idx ->
        { model with
              SelectedTile = Some idx
              Moves = [] }
    | SecondTileClicked b ->
...
let InputTile =
    FunctionComponent.Of
        ((fun (props: InputTileProps) ->
            img [ sprintf "./tiles/%d.jpg" (props.Key + 1) |> Src
...
let Input =
    FunctionComponent.Of
        ((fun ({ Model = model; Dispatch = dispatch }: InputProps) ->
...
let moveTileClassName current source target =
    if current = source then "source"
    else if current = target then "target"
    else "invariant"
...
let Move =
    FunctionComponent.Of
        ((fun { Index = idx; Current = current; Source = s; Target = t } ->
            let rows =
                ...
                |> Array.mapi (fun rIdx rows ->
                    let tiles =
                        ...
                        |> Array.map (fun tile ->
                            let key =
                                sprintf "move-%d-row-%d-%d" idx rIdx tile
                            ...
                            div [ Key key
                                  ClassName className
                                  Style [ CSSProp.BackgroundImage url ] ] [])

                    div [ ClassName "move-row" ] tiles)

            div [ ClassName "move" ] [
                yield h5 [] [
                          sprintf "Step %d" (idx + 1) |> str
                      ]
                yield! rows
            ]),
         "Move")
